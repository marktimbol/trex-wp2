...
function av_get_the_heading() {
	...
	$html .= sprintf( apply_filters( 'av_heading', get_option( '_av_heading', __( 'You must be %s years old to visit this site.', 'age_verify' ) ) ), av_get_minimum_age() );
	return $html;
}
...
function av_get_verify_form() {
	...
	if ( $error ) :
		...
		if ( $error == 3 )
			$error_string = apply_filters( 'av_error_text_too_young', __( 'It doesn\'t look like you\'re old enough.', 'age_verify' ) );
		
		// Visitor entered an invalid date
		if ( $error == 4 )
			$error_string = apply_filters( 'av_error_text_bad_date', __( 'Please enter a valid date', 'age_verify' ) );
		
		$form .= '<div class="alert alert-warning">' . esc_html( $error_string ) . '</div>';
	...
	switch ( $input_type ) {
		...
		case 'dropdowns' :
			...
			$form .= '<select name="av_verify_m" id="av_verify_m" class="form-control input-lg" placeholder="Month">';
			...
			$form .= '</select> <select name="av_verify_d" id="av_verify_d" class="form-control input-lg" style="width:100px;" placeholder="Day">';
			...
			$form .= '</select> <select name="av_verify_y" id="av_verify_y" class="form-control input-lg" style="width:100px;" placeholder="Year">';
				
				foreach ( range( 1960, date( 'Y' ) - 10 ) as $year ) :
	...
	$form .= '<input type="submit" class="btn btn-lg btn-danger" name="av_verify" id="av_verify" value="' . esc_attr( $submit_button_label ) . '" />';
	$form .= '</div>';
	$form .= '</form>';
